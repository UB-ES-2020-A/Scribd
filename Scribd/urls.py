...
urlpatterns = [
    ...
    url(
        r"ebookdetail/(?P<pk>[0-9]+)/$", follow, name="ebook_custom_detail"
    ),  # everybody
    url(
    ...
    url(
        "provider/", ebook_create_view, name="provider_page"
    ),  # only self provider!!!!!
    ...
    url(
        r"^profile/(?P<username>\w+)/edit/$", edit_profile_page, name="edituserprofile"
    ),  # only self
    url(
