...
urlpatterns = [
    ...
    url(
        r"ebookdetail/(?P<book_k>[0-9]+)/forum/(?P<forum_k>[0-9]+)/$",
        ebook_forum,
        name="forumdetail",
    ),
    url("changebook/(?P<pk>[0-9]+)/$",
        views.change_ebook,
        name="Ebook_change_details"),
    ...
    url("provider/", ebook_create_view,
        name="provider_page"),  # only self provider!!!!!
    url("contract/", contract_page,
        name="contract_page"),  # only self provider!!!!!
    ...
    url(
        ...
        user_profile_page.as_view(),
        name="userprofilepage",
    ),  # only self
    url(r"^profile/(?P<username>\w+)/edit/$",
        edit_profile_page,
        name="edituserprofile"),  # only self
    url(
        r"^profile/(?P<username>\w+)/upgrade/$",
        upgrade_account_view,
        name="upgradeaccount",
    ),  # only self
    url(
        r"^profile/(?P<username>\w+)/updatepayment/$",
        update_payment_details,
        name="updatepayment",
    ),
    url(
        r"^profile/(?P<username>\w+)/cancelconfirmation/$",
        downgrade_account_view,
